        ...
        <div ref="cardsContainer" class="ml-64  flex overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']" >
          <div class="flex-shrink-0">
            <div ref="cardsList" class="flex space-x-4">
              <!-- Cards loop -->
              <div v-for="(card, index) in cards" :key="index" ref="card" class="w-64 bg-white rounded-md shadow-md p-4" :style="`background-color: ${card.color}`">
  ...
  export default {
    data() {
      return {
        ...
        colors: ["yellow", "blue", "red"], // List of colors for card background
    ...
    mounted() {
      ...
      this.cards = Array.from({ length: 10 }, (_, index) => ({
        ...
        color: this.colors[Math.floor(Math.random() * this.colors.length)], // Randomly select a color from the list
